...
.rec-source a:hover { color: #a5b4fc !important; border-bottom-color: #6366f1; }
.rec-source a { cursor: pointer; }
...
.src-modal-overlay.active { display: flex; }
.src-modal {
    background: #111118;
    border: 1px solid #3730a3;
    border-radius: 16px;
...
.src-modal-body strong { color: #e2e8f0; }
.src-modal-footer {
    display: flex;
...
.src-modal-close:hover { background: #2d2a6e; }
...
# SOURCE_LINKS đã được thay thế bằng SOURCE_MODAL_CONTENT bên dưới (popup nội tuyến)

# ─────────────────────────────────────────────────────────────────────────────
# NỘI DUNG MODAL — mỗi nguồn có tiêu đề + mô tả đầy đủ + tổ chức
# ─────────────────────────────────────────────────────────────────────────────
SOURCE_MODAL_CONTENT = {
    'American Heart Association': {
        'org': 'American Heart Association (AHA)',
        'title': 'Hướng dẫn Phòng ngừa Bệnh tim mạch',
        'body': (
            'AHA khuyến nghị <strong>ít nhất 150 phút/tuần hoạt động aerobic cường độ vừa</strong> '
            'hoặc 75 phút/tuần cường độ cao. Kết hợp bài tập tăng cường cơ bắp ≥2 ngày/tuần.<br><br>'
            'Về dinh dưỡng: ưu tiên rau củ, trái cây, ngũ cốc nguyên hạt, protein thực vật và cá. '
            'Hạn chế muối &lt;2.300 mg/ngày, đường thêm &lt;6%, chất béo bão hòa &lt;10% tổng calo.<br><br>'
            'Kiểm soát huyết áp, cholesterol và đường huyết định kỳ là nền tảng phòng ngừa bệnh tim.'
        ),
    },
    'AHA/ACC Guidelines 2019': {
        'org': 'American Heart Association / American College of Cardiology',
        'title': 'Hướng dẫn Phòng ngừa Tim mạch Sơ cấp 2019',
        'body': (
            'Hướng dẫn AHA/ACC 2019 nhấn mạnh <strong>chế độ ăn DASH và Địa Trung Hải</strong> '
            'là bằng chứng mạnh nhất giảm nguy cơ tim mạch.<br><br>'
            'Cụ thể: ăn nhiều rau củ đa dạng, trái cây, nguyên hạt, protein thực vật, '
            'cá/hải sản, sữa ít béo. Hạn chế muối &lt;2.300 mg/ngày (tốt nhất &lt;1.500 mg), '
            'đường thêm &lt;6%, chất béo bão hòa &lt;10%.<br><br>'
            'Statin được khuyến nghị cho người có LDL-C cao hoặc nguy cơ tim mạch 10 năm ≥7,5%.'
        ),
    },
    'WHO Cardiovascular Guidelines': {
        'org': 'World Health Organization (WHO)',
        'title': 'Hướng dẫn Phòng ngừa Bệnh Tim mạch — WHO',
        'body': (
            '<strong>Thuốc lá là yếu tố nguy cơ hàng đầu</strong> gây bệnh tim mạch toàn cầu. '
            'WHO khuyến cáo tránh hoàn toàn thuốc lá và khói thuốc thụ động.<br><br>'
            'Bỏ thuốc lá làm giảm nguy cơ nhồi máu cơ tim ngay trong vòng 1 năm đầu. '
            'Sau 15 năm không hút, nguy cơ tim mạch trở về gần bằng người chưa từng hút thuốc.<br><br>'
            'WHO cũng khuyến nghị hạn chế rượu bia, duy trì cân nặng hợp lý và hoạt động thể chất đều đặn.'
        ),
    },
    'CDC Heart Disease Prevention': {
        'org': 'Centers for Disease Control and Prevention (CDC)',
        'title': 'Phòng ngừa Bệnh Tim — CDC',
        'body': (
            'CDC khuyến nghị duy trì <strong>BMI lý tưởng 18,5–24,9 kg/m²</strong>. '
            'Vòng eo &lt;102 cm (nam) hoặc &lt;88 cm (nữ) để giảm nguy cơ tim mạch.<br><br>'
            'Béo phì là yếu tố nguy cơ độc lập của bệnh tim, tăng huyết áp, đái tháo đường type 2 '
            'và rối loạn lipid máu — tất cả đều ảnh hưởng trực tiếp đến sức khỏe tim mạch.<br><br>'
            'CDC khuyến nghị giảm 5–10% cân nặng đã cải thiện đáng kể các chỉ số tim mạch.'
        ),
    },
    'ACC/AHA Prevention Guidelines': {
        'org': 'American College of Cardiology / American Heart Association',
        'title': 'Khám Sàng lọc Tim mạch Định kỳ',
        'body': (
            'ACC/AHA khuyến nghị <strong>đo huyết áp, cholesterol và đường huyết hàng năm</strong> '
            'cho người trưởng thành từ 20 tuổi trở lên.<br><br>'
            'Sử dụng thang điểm <em>Pooled Cohort Equations</em> để đánh giá nguy cơ tim mạch 10 năm. '
            'Nếu nguy cơ ≥7,5%: xem xét statin và can thiệp lối sống tích cực.<br><br>'
            'Tầm soát sớm giúp phát hiện và kiểm soát yếu tố nguy cơ trước khi xảy ra biến cố tim mạch.'
        ),
    },
    'AHA Stress Management': {
        'org': 'American Heart Association (AHA)',
        'title': 'Quản lý Căng thẳng và Sức khỏe Tim mạch',
        'body': (
            'Căng thẳng mãn tính làm tăng cortisol, gây viêm mạch máu, tăng huyết áp và '
            'tăng nguy cơ nhồi máu cơ tim. AHA khuyến nghị:<br><br>'
            '• <strong>Thiền định và mindfulness</strong>: 10–20 phút/ngày giảm huyết áp tâm thu 4–5 mmHg<br>'
            '• <strong>Yoga và hít thở sâu</strong>: kích hoạt hệ thần kinh phó giao cảm<br>'
            '• <strong>Ngủ đủ 7–9 giờ/đêm</strong>: thiếu ngủ tăng gấp đôi nguy cơ tim mạch<br>'
            '• Hoạt động thể chất đều đặn là phương pháp giảm stress hiệu quả nhất.'
        ),
    },
    'ACC/AHA Diagnostic Guidelines': {
        'org': 'American College of Cardiology / American Heart Association',
        'title': 'Các Xét nghiệm Chẩn đoán Tim mạch Cần Làm',
        'body': (
            'ACC/AHA khuyến nghị các xét nghiệm chuyên sâu khi có nguy cơ bệnh tim:<br><br>'
            '• <strong>ECG 12 chuyển đạo</strong>: phát hiện rối loạn nhịp, thiếu máu cơ tim<br>'
            '• <strong>Siêu âm tim (Echo)</strong>: đánh giá cấu trúc, chức năng tim và van tim<br>'
            '• <strong>Stress test</strong>: phát hiện thiếu máu cơ tim khi gắng sức<br>'
            '• <strong>Lipid máu chi tiết</strong>: LDL-C, HDL-C, triglycerid, non-HDL<br>'
            '• <strong>HbA1c, CRP (viêm), creatinine</strong>: đánh giá yếu tố nguy cơ đi kèm<br>'
            '• CT/MRI tim nếu cần theo chỉ định bác sĩ chuyên khoa.'
        ),
    },
    'WHO Medication Adherence': {
        'org': 'World Health Organization (WHO)',
        'title': 'Tuân thủ Điều trị — Nền tảng Kiểm soát Bệnh Tim',
        'body': (
            'WHO xác định <strong>tuân thủ điều trị kém là nguyên nhân hàng đầu</strong> '
            'thất bại trong kiểm soát bệnh mãn tính, bao gồm bệnh tim mạch.<br><br>'
            'Các thuốc tim mạch thường dùng: statin (hạ cholesterol), '
            'thuốc huyết áp (ACE inhibitor, ARB, beta-blocker), aspirin liều thấp.<br><br>'
            '<strong>TUYỆT ĐỐI không tự ý ngừng thuốc</strong> dù cảm thấy khỏe — '
            'ngừng đột ngột có thể gây nhồi máu cơ tim phục hồi (rebound effect). '
            'Mọi thay đổi phác đồ phải có ý kiến bác sĩ.'
        ),
    },
    'AHA Risk Factor Management': {
        'org': 'American Heart Association (AHA)',
        'title': 'Loại bỏ Yếu tố Nguy cơ Tim mạch',
        'body': (
            'AHA xác định các yếu tố nguy cơ có thể thay đổi được:<br><br>'
            '• <strong>BỎ THUỐC LÁ ngay lập tức</strong> — giảm nguy cơ tim mạch 50% trong 1 năm<br>'
            '• <strong>Hạn chế rượu bia</strong>: không quá 1 đơn vị/ngày (nữ), 2 đơn vị/ngày (nam)<br>'
            '• <strong>Hạn chế caffeine</strong>: không quá 400 mg/ngày (khoảng 4 tách cà phê)<br>'
            '• <strong>Giảm muối</strong>: &lt;1.500 mg/ngày nếu có tăng huyết áp<br>'
            '• <strong>Tránh chất béo trans và bão hòa</strong>: thay bằng chất béo không bão hòa đơn/đa'
        ),
    },
    'Emergency Cardiovascular Care': {
        'org': 'American Heart Association — Emergency Cardiovascular Care',
        'title': 'Nhận biết Dấu hiệu Cấp cứu Tim mạch',
        'body': (
            '<strong style="color:#f87171;">GỌI CẤP CỨU 115 NGAY khi có các dấu hiệu sau:</strong><br><br>'
            '• Đau ngực, tức ngực, cảm giác bị đè nặng<br>'
            '• Khó thở, thở gấp đột ngột<br>'
            '• Đau lan ra cánh tay trái, vai, cổ, hàm, lưng<br>'
            '• Mệt mỏi cực độ, đổ mồ hôi lạnh bất thường<br>'
            '• Chóng mặt, ngất xỉu, tim đập không đều<br><br>'
            '<strong>Thời gian vàng trong nhồi máu cơ tim là 90 phút</strong> — '
            'can thiệp tái thông mạch trong thời gian này cứu sống cơ tim. '
            'Đừng tự lái xe đến bệnh viện, hãy gọi 115.'
        ),
    },
    'AHA Hypertension Guidelines': {
        'org': 'American Heart Association (AHA)',
        'title': 'Kiểm soát Tăng huyết áp — Hướng dẫn AHA',
        'body': (
            'Tăng huyết áp ≥140/90 mmHg làm tăng nguy cơ nhồi máu cơ tim và đột quỵ gấp 2–3 lần.<br><br>'
            '<strong>Biện pháp không dùng thuốc (trước tiên):</strong><br>'
            '• Giảm muối &lt;1.500 mg/ngày (giảm 5–6 mmHg huyết áp tâm thu)<br>'
            '• Tăng kali từ rau củ, trái cây, chuối, khoai lang<br>'
            '• Giảm cân: mỗi 1 kg giảm → huyết áp giảm ~1 mmHg<br>'
            '• Tập thể dục aerobic 30 phút/ngày × 5 ngày/tuần<br><br>'
            '<strong>Khi cần thuốc:</strong> ACE inhibitor, ARB, thiazide diuretic hoặc calcium channel blocker '
            'theo chỉ định bác sĩ. Mục tiêu huyết áp &lt;130/80 mmHg.'
        ),
    },
    'ACC/AHA Cholesterol Guidelines': {
        'org': 'American College of Cardiology / American Heart Association',
        'title': 'Kiểm soát Cholesterol Cao — Hướng dẫn ACC/AHA',
        'body': (
            'Cholesterol ≥240 mg/dL là nguy cơ cao bệnh tim mạch vữa xơ. '
            'ACC/AHA khuyến nghị:<br><br>'
            '<strong>Thay đổi lối sống trước:</strong><br>'
            '• Giảm chất béo bão hòa xuống &lt;6% tổng calo<br>'
            '• Loại bỏ hoàn toàn trans fat (margarine, đồ chiên rán công nghiệp)<br>'
            '• Tăng chất xơ hòa tan: yến mạch, đậu, táo, lê (giảm LDL-C 5–10%)<br>'
            '• Thực phẩm bổ sung plant sterol/stanol 2g/ngày<br><br>'
            '<strong>Statin được chỉ định</strong> khi LDL-C ≥190 mg/dL, hoặc nguy cơ tim mạch 10 năm ≥7,5%. '
            'Statin cường độ cao (rosuvastatin, atorvastatin) giảm LDL-C 50–60%.'
        ),
    },
    'CDC Healthy Weight': {
        'org': 'Centers for Disease Control and Prevention (CDC)',
        'title': 'Duy trì Cân nặng Khỏe mạnh — CDC',
        'body': (
            'CDC khuyến nghị tiếp cận giảm cân <strong>bền vững và khoa học</strong>:<br><br>'
            '• Mục tiêu: giảm 0,5–1 kg/tuần (không giảm nhanh hơn)<br>'
            '• Tạo thâm hụt calo 500–1.000 kcal/ngày qua ăn ít hơn + vận động nhiều hơn<br>'
            '• Không bỏ bữa — ăn 3 bữa chính, hạn chế bữa phụ giàu calo<br>'
            '• Theo dõi cân nặng hàng tuần để điều chỉnh kịp thời<br><br>'
            '<strong>Tham khảo chuyên gia dinh dưỡng</strong> để xây dựng thực đơn cá nhân hóa. '
            'Giảm 5–10% cân nặng đã cải thiện đáng kể huyết áp, cholesterol và đường huyết.'
        ),
    },
}
...
def source_link(name):
    # Tạo id an toàn từ tên nguồn
    safe_id = name.replace(' ', '_').replace('/', '_').replace('.', '').replace('(', '').replace(')', '')
    return (
        f'<a href="javascript:void(0)" '
        f'onclick="showSrcModal(\'{safe_id}\')" '
        f'style="cursor:pointer;">{name}</a>'
    )


def build_modal_html():
    """Tạo tất cả modal divs + JavaScript"""
    modals_html = ''
    for name, info in SOURCE_MODAL_CONTENT.items():
        safe_id = name.replace(' ', '_').replace('/', '_').replace('.', '').replace('(', '').replace(')', '')
        modals_html += f"""
<div class="src-modal-overlay" id="modal_{safe_id}" onclick="if(event.target===this)closeSrcModal('{safe_id}')">
  <div class="src-modal">
    <div class="src-modal-logo">{info['org'].split('(')[-1].replace(')','').strip() if '(' in info['org'] else info['org'][:3].upper()}</div>
    <div class="src-modal-title">{info['title']}</div>
    <div class="src-modal-body">{info['body']}</div>
    <div class="src-modal-footer">
      <div class="src-modal-org">{info['org']}</div>
      <button class="src-modal-close" onclick="closeSrcModal('{safe_id}')">Đóng</button>
    </div>
  </div>
</div>"""
...
def build_recommendations(is_healthy, pd_, bmi):
  ...
  <div class="rec-sub">Dua tren huong dan tu American Heart Association (AHA), American College of Cardiology (ACC), World Health Organization (WHO), Centers for Disease Control (CDC) — <em>Bam vao ten nguon de xem chi tiet</em></div>
  {items}
