...
# ─────────────────────────────────────────────────────────────────────────────
# SOURCE LINKS — map ten nguon -> URL thuc te
# ─────────────────────────────────────────────────────────────────────────────
SOURCE_LINKS = {
    'American Heart Association':
        'https://www.heart.org/en/health-topics/heart-attack/prevention',
...
def show_training_panel():
    ...
    st.markdown('<div class="card-title">Huan luyen Mo hinh D-DOA v4 + SVM-RBF</div>', unsafe_allow_html=True)

    st.markdown("""
<div style="background:#0a0f1a;border:1px solid #1e2d3d;border-left:3px solid #4f46e5;
     border-radius:8px;padding:12px 16px;margin-bottom:16px;font-size:0.82rem;color:#7dd3fc;">
<strong style="color:#a5b4fc;">Thông số DeepDOA v4</strong> (đồng bộ Heart1_fixed_v4_smote_fixed)<br>
logC∈[0,4] | logγ∈[-4,1] | alpha∈[0.90→0.20] | bl∈[0.35→0.80] | elite_k=7 | stagnation=10<br>
<span style="color:#64748b;">Fitness: Pipeline(StandardScaler→SVM) + scoring=roc_auc | Smart init: 6 droplets seeded</span>
</div>""", unsafe_allow_html=True)

    c1, c2, c3 = st.columns(3)
    with c1: n_droplets = st.slider("n_droplets", 10, 50, 30)
    with c2: n_iter     = st.slider("n_iter",     20, 150, 60)
    with c3: cv_folds   = st.slider("CV Folds",   3,  10,   5)

    if st.button("Bat dau Huan luyen", type="primary", use_container_width=True):
        with st.spinner("Dang tai du lieu UCI Heart Disease (KNN Imputer)..."):
            try:
                X_raw, y, FEATURES, data = load_data()
            except Exception as e:
                st.error(f"Khong tim thay du lieu!\n\nTai ve tu UCI va giai nen vao:\n"
                         f"`~/Downloads/heart+disease/`\n\nLoi: {e}")
                st.stop()
        st.success(f"{len(y)} mau · {len(FEATURES)} dac trung (13 features day du)")
        with st.spinner("Ap dung SMOTE (k_neighbors=3)..."):
            X_bal, y_bal, smote_ok = apply_smote(X_raw, y)
        if smote_ok:
            st.info(f"SMOTE: {X_raw.shape[0]} -> {X_bal.shape[0]} mau (can bang)")
        st.markdown("**Dang chay D-DOA v4 Optimization (scoring=roc_auc)...**")
        prog = st.progress(0); stat = st.empty()
        doa = DeepDOA(n_droplets=n_droplets, n_iter=n_iter, cv_folds=cv_folds)
        doa.optimize(X_bal, y_bal, prog=prog, stat=stat)
        prog.progress(1.0); stat.empty()
        best_w, best_C, best_gam = doa.best_weights_, doa.best_C_, doa.best_gamma_
        X_w = X_bal * best_w
        X_tr, X_te, y_tr, y_te = train_test_split(
            X_w, y_bal, test_size=0.20, random_state=SEED, stratify=y_bal
        )
        sc = StandardScaler()
        ...
        st.session_state.metrics = dict(
            best_w=best_w, best_C=best_C, best_gam=best_gam,
            best_cv=doa.best_fitness_,   # CV AUC-ROC (1 - loss)
            acc=accuracy_score(y_te, yp),
            auc=roc_auc_score(y_te, yprob),
            f1=f1_score(y_te, yp),
            mcc=matthews_corrcoef(y_te, yp),
            n_feat=len(FEATURES), FEATURES=FEATURES,
            scaler=sc, svm=svm, doa_hist=doa.hist_best
        )
...
<div class="card">
  ...
  <div class="doa-bar">D-DOA v4: C={m['best_C']:.4f} | γ={m['best_gam']:.6f} | CV AUC={m['best_cv']:.4f} | Test AUC={m['auc']:.4f}</div>
    ...
    with btn1: submit = st.button("Phan tich", type="primary", use_container_width=True)
    with btn2:
...
<div style="display:flex;justify-content:space-between;align-items:center;
  ...
  <div style="font-size:0.85rem;color:#94a3b8;">
    ...
    File: <strong style="color:#a5b4fc;">{uploaded.name}</strong>
        ...
        if st.button(f"Phan tich {n_valid} benh nhan", type="primary", use_container_width=True):
  ...
  <table class="batch-tbl">
    ...
    <tbody>
      {rows_html}
        ...
        if st.button("Xoa ket qua batch", use_container_width=False):
